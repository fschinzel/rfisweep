...
# Extract data from SDM to determine whether a VLASS observations has completed
# Or to determine where it stopped recording data.
# -- Frank Schinzel (07/29/2020)
...
def read_metadata(sdmname):
  sdm = sdmpy.SDM(sdmname,lazy=True)
  scandict = {}; sourcedict={}
...
print "Checking for scans with less than 2 integrations (does not show >2 nint)\n"
first = False
count = 0
...
#		if 24000000.0!=round(os.stat(sc[i]['bdfstr']).st_size,-6):
 #               	print os.stat(sc[i]['bdfstr']).st_size, sc[i]['scan'], sc[i]['subscan']
		...
		if sc[i]['nints'] < 2:
			if first == False:
				print "Found scans with missing integrations starting at:"
				...
				try:
					...
					last = sc[i+j-1]
				print "Number of ints: %s, in scan/subscan: %s/%s" %  (bad['nints'],bad['scan'],bad['subscan'])
				print "Field: %s" % bad['source']
				print "Starting at: %s" % bad['startmjd']
				first = True
				count += 1
...
if count != 0:
	print "%i subsequent subscans found with missing integrations!" % (count-1)
else:
	print "No subscans with less than 2 integrations found. Yay!"
